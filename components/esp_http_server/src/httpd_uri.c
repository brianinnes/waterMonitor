...
esp_err_t httpd_register_404_handler(httpd_handle_t handle,
                                     const httpd_uri_t *uri_handler)
{
    if (handle == NULL || uri_handler == NULL) {
        return ESP_ERR_INVALID_ARG;
     ...
     */
    if (NULL != hd->hd_404) {
        ESP_LOGW(TAG, LOG_FMT("404 handler already registered"));
        return ESP_ERR_HTTPD_HANDLER_EXISTS;
    }

   
    hd->hd_404 = malloc(sizeof(httpd_uri_t));
    if (hd->hd_404 == NULL) {
        /* Failed to allocate memory */
        return ESP_ERR_HTTPD_ALLOC_MEM;
    }

            /* Clear URI string, as not used for 404 handler */
    hd->hd_404->uri = NULL;

    /* Copy remaining members */
    hd->hd_404->method   = uri_handler->method;
    hd->hd_404->handler  = uri_handler->handler;
    hd->hd_404->user_ctx = uri_handler->user_ctx;
    ESP_LOGD(TAG, LOG_FMT("404 handler installed"));
    return ESP_OK;
}

esp_err_t httpd_unregister_404_handler(httpd_handle_t handle,
                                       const char *uri, httpd_method_t method)
{
    if (handle == NULL || uri == NULL) {
        return ESP_ERR_INVALID_ARG;
    ...
    if (NULL != hd->hd_404) {
        ESP_LOGD(TAG, LOG_FMT("removing 404 handler"));
        free(hd->hd_404);
        hd->hd_404 = NULL;
        return ESP_OK;
    }
    ESP_LOGW(TAG, LOG_FMT("404 handler not registered"));
    return ESP_ERR_NOT_FOUND;
}

// Code from https://www.esp32.com/viewtopic.php?t=1139&start=10
#ifndef ESP32
#define ESP32
#endif
...
static void captdnsRecv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length)
{
    ...
    for (i = 0; i < my_ntohs(&hdr->qdcount); i++)
    {
        ...
        if (my_ntohs(&qf->type) == QTYPE_A)
        {
            ...
            tcpip_adapter_get_ip_info(/*TCPIP_ADAPTER_IF_ETH*/ TCPIP_ADAPTER_IF_AP, &info);
            //struct ip_info info;
...
{
    xTaskCreate(captdnsTask, (const char *)"captdns_task", 4096, NULL, 3, NULL);
}
